  ...
  <div id="users">
    ...
    <el-card>
      ...
      <el-form :model="Addform" label-position="top" :rules="rules" ref="addform">
        <el-tabs tab-position="left" v-model="activeIndex" @tab-click="tagChecked">
          ...
          <el-tab-pane label="商品内容" name="4">
            ...
            <el-button type="primary" @click="preventUP" style="margin-top:15px">编辑商品</el-button>
          </el-tab-pane>
        </el-tabs>
      </el-form>
    </el-card>
...
export default {
  ...
  methods: {
    ...
    async goodsinfo() {
      ...
      this.Addform.goods_cat = this.Addform.goods_cat.split(",").map(Number);
      this.cateId = res.data.cat_id;
      this.Addform.pics.map((item, index) => {
        let filepic = { name: index, url: item.pics_mid_url };
        this.fileList.push(filepic);
      });
    },
    //在选择条被选中的时候触发
    async tagChecked() {
    ...
    // 图片预览
    handlePreview(file) {
      this.previmgURL = file.url;
      this.dialogVisible = true;
    },
    // 图片上传
    handleRemove(files) {
      console.log(files);
      let removepath = files.url;
      // 获取将要删除的图片的临时路径 从pics数组中，找到这个对应的索引值，调用数组的splice方法，把图片信息对象，从pics数组中移出
      let i = this.Addform.pics.findIndex(x => x.pics_mid_url == removepath);
    ...
    preventUP() {
      ...
      this.$refs.addform.validate(async valid => {
        if (valid) {
          ...
          // 发送请求
          let { data: res } = await this.$axios.post(`goods${id}`, form);
